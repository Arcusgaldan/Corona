...
def formataImpressao(tabela):
    tabela = mascaraDataEsus(tabela)
    return tabela[["NUmero da NotificaCAo", "Nome Completo", "CPF", "CNS", "Data de Nascimento", "Nome Completo da MAe", "Sexo", "Telefone de Contato", "Telefone Celular", "CEP", "Logradouro", "NUmero", "Complemento", "Bairro", "RaCa/Cor", "Data da NotificaCAo", "Data do inIcio dos sintomas", "Notificante CNES", "Notificante Email", "Notificante Nome Completo"]]
...
filtroTotalOutras10 = tabelaTotalEsus['Notificante CNES'] == 419907
filtroTotalOutras11 = tabelaTotalEsus['Notificante Email'] == "maicon.douglas19@hotmail.com" #Email da Drogaria São Paulo, que não possui CNES
filtroTotalOutras12 = tabelaTotalEsus['Notificante Email'] == "yaah_cardoso@hotmail.com"
filtroTotalOutras13 = tabelaTotalEsus['Notificante Email'] == "robertamap@yahoo.com.br"
filtroTotalOutras14 = tabelaTotalEsus['Notificante Email'] == "barretos2@dpsp.com.br"
filtroTotalOutras15 = tabelaTotalEsus['Notificante Email'] == "marcela@ltaseguranca.com.br"
filtroTotalOutras16 = tabelaTotalEsus['Notificante CNES'] == 2074176
...
tabelaTotalOutras = tabelaTotalEsus.where(filtroTotalOutras1 | filtroTotalOutras2 | filtroTotalOutras3 | filtroTotalOutras4 | filtroTotalOutras5 | filtroTotalOutras6 | filtroTotalOutras7 | filtroTotalOutras8 | filtroTotalOutras9 | filtroTotalOutras10 | filtroTotalOutras11 | filtroTotalOutras12 | filtroTotalOutras13 | filtroTotalOutras14 | filtroTotalOutras15 | filtroTotalOutras16).dropna(how='all')
...
filtroNegStc1 = tabelaTotalStc["Resultado Totais"] == "NAo Reagente"
filtroNegStc2 = tabelaTotalStc["Resultado IgA"] == "NAo Reagente"
filtroNegStc3 = tabelaTotalStc["Resultado (PCR/RApidos)"] == "Negativo"
filtroNegStc4 = tabelaTotalStc["Resultado IgM"] == "NAo Reagente"

#Filtros para resultados negativos do Pio XII
filtroNegPio1 = tabelaTotalPio["Resultado Totais"] == "NAo Reagente"
filtroNegPio2 = tabelaTotalPio["Resultado IgA"] == "NAo Reagente"
filtroNegPio3 = tabelaTotalPio["Resultado (PCR/RApidos)"] == "Negativo"
filtroNegPio4 = tabelaTotalPio["Resultado IgM"] == "NAo Reagente"

#Filtros para resultados negativos das Outras Unidades
filtroNegOut1 = tabelaTotalOutras["Resultado Totais"] == "NAo Reagente"
filtroNegOut2 = tabelaTotalOutras["Resultado IgA"] == "NAo Reagente"
filtroNegOut3 = tabelaTotalOutras["Resultado (PCR/RApidos)"] == "Negativo"
filtroNegOut4 = tabelaTotalOutras["Resultado IgM"] == "NAo Reagente"
...
tabelaPositivosStc = tabelaTotalStc.where(filtroPosStc1 | filtroPosStc2 | filtroPosStc3 | filtroPosStc4).dropna(how='all').drop_duplicates("CPF", ignore_index=True, keep='last')
tabelaPositivosPio = tabelaTotalPio.where(filtroPosPio1 | filtroPosPio2 | filtroPosPio3 | filtroPosPio4).dropna(how='all').drop_duplicates("CPF", ignore_index=True, keep='last')
tabelaPositivosOut = tabelaTotalOutras.where(filtroPosOut1 | filtroPosOut2 | filtroPosOut3 | filtroPosOut4).dropna(how='all').drop_duplicates("CPF", ignore_index=True, keep='last')

endPositivos = timer()
print("Terminei de separar os positivos aos " + str(endPositivos - start) + " segundos.")

#Aplica os filtros negativos definidos acima
tabelaNegativosStc = tabelaTotalStc.where(filtroNegStc1 | filtroNegStc2 | filtroNegStc3 | filtroNegStc4).dropna(how='all')
...
tabelaNegativosPio = tabelaTotalPio.where(filtroNegPio1 | filtroNegPio2 | filtroNegPio3 | filtroNegPio4).dropna(how='all')
...
tabelaNegativosOut = tabelaTotalOutras.where(filtroNegOut1 | filtroNegOut2 | filtroNegOut3 | filtroNegOut4).dropna(how='all')
...
for row in tabelaPositivosOut.itertuples():
    notifAssessor = acharId(row.CPF, row.CNS, row[paramColunaNomeEsus], row[paramColunaDataNascEsus], tabelaTotalAssessor)    
    if "CONFIRMADO" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaPositivosOutFalso, row, "CONFIRMADO DA MESMA UNIDADE")
        tabelaPositivosOut.drop(row.Index, inplace=True)
        continue

for row in tabelaNegativosOut.itertuples():    
    notifAssessor = acharId(row.CPF, row.CNS, row[paramColunaNomeEsus], row[paramColunaDataNascEsus], tabelaTotalAssessor)
    if "CONFIRMADO" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaNegativosOutFalso, row, "CONFIRMADO DA MESMA UNIDADE")
        tabelaNegativosOut.drop(row.Index, inplace=True)
        continue
    if "NEGATIVO" in notifAssessor["SituaCAo"].values:
        negativosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "NEGATIVO").dropna(how='all')
        for rowNegativo in negativosAssessor.itertuples():
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "NEGATIVO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus]})
            appendTabelaAuxiliar(tabelaNegativosOutFalso, row, "NEGATIVO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaNegativosOut.drop(row.Index, inplace=True)
            continue
    if "DESCARTADO" in notifAssessor["SituaCAo"].values:
        descartadosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "DESCARTADO").dropna(how='all')
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "DESCARTADO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus], "Notificante CNES": row[paramColunaCnesEsus]})
            appendTabelaAuxiliar(tabelaNegativosOutFalso, row, "DESCARTADO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaNegativosOut.drop(row.Index, inplace=True)
            continue

for row in tabelaSuspeitosOut.itertuples():
    notifAssessor = acharId(row.CPF, row.CNS, row[paramColunaNomeEsus], row[paramColunaDataNascEsus], tabelaTotalAssessor)
    if "CONFIRMADO" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaSuspeitosOutFalso, row, "CONFIRMADO DA MESMA UNIDADE")
        tabelaSuspeitosOut.drop(row.Index, inplace=True)
        continue        
    if "SUSPEITA" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaSuspeitosOutFalso, row, "SUSPEITA DA MESMA UNIDADE")
        tabelaSuspeitosOut.drop(row.Index, inplace=True)
        continue
    if "NEGATIVO" in notifAssessor["SituaCAo"].values:
        negativosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "NEGATIVO").dropna(how='all')
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "NEGATIVO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus]})
            appendTabelaAuxiliar(tabelaSuspeitosOutFalso, row, "NEGATIVO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaSuspeitosOut.drop(row.Index, inplace=True)
            continue
    if "DESCARTADO" in notifAssessor["SituaCAo"].values:
        descartadosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "DESCARTADO").dropna(how='all')
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "DESCARTADO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus], "Notificante CNES": row[paramColunaCnesEsus]})
            appendTabelaAuxiliar(tabelaSuspeitosOutFalso, row, "DESCARTADO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaSuspeitosOut.drop(row.Index, inplace=True)
            continue
        
for row in tabelaMonitoramentoOut.itertuples():
    notifAssessor = acharId(row.CPF, row.CNS, row[paramColunaNomeEsus], row[paramColunaDataNascEsus], tabelaTotalAssessor)
    if "CONFIRMADO" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaMonitoramentoOutFalso, row, "CONFIRMADO DA MESMA UNIDADE")
        tabelaMonitoramentoOut.drop(row.Index, inplace=True)
        continue
    if "SUSPEITA" in notifAssessor["SituaCAo"].values:
        appendTabelaAuxiliar(tabelaMonitoramentoOutFalso, row, "SUSPEITA DA MESMA UNIDADE")
        tabelaMonitoramentoOut.drop(row.Index, inplace=True)
        continue
    if "NEGATIVO" in notifAssessor["SituaCAo"].values:
        negativosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "NEGATIVO").dropna(how='all')
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "NEGATIVO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus]})
            appendTabelaAuxiliar(tabelaMonitoramentoOutFalso, row, "NEGATIVO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaMonitoramentoOut.drop(row.Index, inplace=True)
            continue
    if "DESCARTADO" in notifAssessor["SituaCAo"].values:
        descartadosAssessor = notifAssessor.where(notifAssessor["SituaCAo"] == "DESCARTADO").dropna(how='all')
        ...
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "DESCARTADO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus], "Notificante CNES": row[paramColunaCnesEsus]})
            appendTabelaAuxiliar(tabelaMonitoramentoOutFalso, row, "DESCARTADO DENTRO DO PERIODO E DA MESMA UNIDADE")
            tabelaMonitoramentoOut.drop(row.Index, inplace=True)
            continue

endOut = timer()
print("Terminei de processar aos unidades externas aos " + str(endOut - start) + " segundos.")
...
with pd.ExcelWriter('Santa Casa ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerStcCorreto:
    tabelaMonitoramentoStc = formataImpressao(tabelaMonitoramentoStc)
    tabelaMonitoramentoStc.to_excel(writerStcCorreto, "Monitoramento", index=False)    
    
    tabelaSuspeitosStc = formataImpressao(tabelaSuspeitosStc)    
    tabelaSuspeitosStc.to_excel(writerStcCorreto, "Suspeitos", index=False)    
    
    tabelaPositivosStc = formataImpressao(tabelaPositivosStc)    
    tabelaPositivosStc.to_excel(writerStcCorreto, "Positivos", index=False)    
    
    tabelaNegativosStc = formataImpressao(tabelaNegativosStc)    
    tabelaNegativosStc.to_excel(writerStcCorreto, "Negativos", index=False)
    
with pd.ExcelWriter('PIO XII ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerPioCorreto:
    tabelaMonitoramentoPio = formataImpressao(tabelaMonitoramentoPio)    
    tabelaMonitoramentoPio.to_excel(writerPioCorreto, "Monitoramento", index=False)    
    
    tabelaSuspeitosPio = formataImpressao(tabelaSuspeitosPio)    
    tabelaSuspeitosPio.to_excel(writerPioCorreto, "Suspeitos", index=False)    
    
    tabelaPositivosPio = formataImpressao(tabelaPositivosPio)    
    tabelaPositivosPio.to_excel(writerPioCorreto, "Positivos", index=False)    
    
    tabelaNegativosPio = formataImpressao(tabelaNegativosPio)    
    tabelaNegativosPio.to_excel(writerPioCorreto, "Negativos", index=False)
    
with pd.ExcelWriter('HANS ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerHansCorreto:
    tabelaMonitoramentoHans = formataImpressao(tabelaMonitoramentoHans)    
    tabelaMonitoramentoHans.to_excel(writerHansCorreto, "Monitoramento", index=False)    
    
    tabelaSuspeitosHans = formataImpressao(tabelaSuspeitosHans)    
    tabelaSuspeitosHans.to_excel(writerHansCorreto, "Suspeitos", index=False)    
    
    tabelaPositivosHans = formataImpressao(tabelaPositivosHans)    
    tabelaPositivosHans.to_excel(writerHansCorreto, "Positivos", index=False)
    
    tabelaNegativosHans = formataImpressao(tabelaNegativosHans)    
    tabelaNegativosHans.to_excel(writerHansCorreto, "Negativos", index=False)
    
with pd.ExcelWriter('Externas ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerOutCorreto:
    tabelaMonitoramentoOut = formataImpressao(tabelaMonitoramentoOut)    
    tabelaMonitoramentoOut.to_excel(writerOutCorreto, "Monitoramento", index=False)    
    
    tabelaSuspeitosOut = formataImpressao(tabelaSuspeitosOut)    
    tabelaSuspeitosOut.to_excel(writerOutCorreto, "Suspeitos", index=False)    
    
    tabelaPositivosOut = formataImpressao(tabelaPositivosOut)    
    tabelaPositivosOut.to_excel(writerOutCorreto, "Positivos", index=False)
    
    tabelaNegativosOut = formataImpressao(tabelaNegativosOut)    
    tabelaNegativosOut.to_excel(writerOutCorreto, "Negativos", index=False) 
...
with pd.ExcelWriter('INCORRETO ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerIncorreto:
    ...
    tabelaMonitoramentoOutFalso = pd.DataFrame(tabelaMonitoramentoOutFalso)
    tabelaMonitoramentoOutFalso.to_excel(writerIncorreto, "Monitoramento Externas")    
    ...
    tabelaSuspeitosOutFalso = pd.DataFrame(tabelaSuspeitosOutFalso)
    tabelaSuspeitosOutFalso.to_excel(writerIncorreto, "Suspeitos Externas")    
    ...
    tabelaPositivosOutFalso = pd.DataFrame(tabelaPositivosOutFalso)
    tabelaPositivosOutFalso.to_excel(writerIncorreto, "Positivos Externas")    
    ...
    tabelaNegativosOutFalso = pd.DataFrame(tabelaNegativosOutFalso)
    tabelaNegativosOutFalso.to_excel(writerIncorreto, "Negativos Externas")    
