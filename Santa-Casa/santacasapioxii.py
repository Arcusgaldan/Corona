...
for row in tabelaSuspeitosOut.itertuples():
    ...
    if "CONFIRMADO" in notifAssessor["SituaCAo"].values:
        ...
        for rowPositivo in positivosAssessor.itertuples():
            flagIsWrong = False
            unidade = acharUnidade(row)
            if rowPositivo.Unidade == unidade:
                flagIsWrong = True
                break
        if flagIsWrong:            
    ...
    if "SUSPEITA" in notifAssessor["SituaCAo"].values:
        ...
        for rowSuspeito in suspeitosAssessor.itertuples():
            flagIsWrong = False
            unidade = acharUnidade(row)
            if rowSuspeito.Unidade == unidade:
                flagIsWrong = True
                break
        if flagIsWrong:            
    ...
    if "NEGATIVO" in notifAssessor["SituaCAo"].values:
        ...
        for rowNegativo in negativosAssessor.itertuples():
            ...
            dataNotifEsus = row[paramColunaDataNotifEsus]
            unidade = acharUnidade(row)
            if rowNegativo.Unidade == unidade and (dataNotifAssessor >= dataNotifEsus - timedelta(days=paramDiasNegativo)):
                flagIsWrong = True
                break
        if flagIsWrong:
            #tabelaNegativosPioFalso.append({"Nome": row[paramColunaNomeEsus], "CPF": row.CPF, "Motivo": "NEGATIVO DENTRO DO PERIODO", "Data Not.": row[paramColunaDataNotifEsus]})
...
with pd.ExcelWriter('Externas ' + paramDataAtual.strftime("%d.%m") + '.xlsx') as writerOutCorreto:
    tabelaMonitoramentoOut = mascaraDataEsus(tabelaMonitoramentoOut)    
    tabelaMonitoramentoOut.to_excel(writerOutCorreto, "Monitoramento")    
    
    tabelaSuspeitosOut = mascaraDataEsus(tabelaSuspeitosOut)    
    tabelaSuspeitosOut.to_excel(writerOutCorreto, "Suspeitos")    
    
    tabelaPositivosOut = mascaraDataEsus(tabelaPositivosOut)    
    tabelaPositivosOut.to_excel(writerOutCorreto, "Positivos")
    
    tabelaNegativosOut = mascaraDataEsus(tabelaNegativosOut)    
    tabelaNegativosOut.to_excel(writerOutCorreto, "Negativos") 
