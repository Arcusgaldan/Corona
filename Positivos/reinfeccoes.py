...
paramDiasReinfeccao = 90

tabelaTotal = pd.read_excel("lista total.xlsx").sort_values(by=["Cód. Paciente", "Data da Notificação"], ignore_index=True) #Lista total dos agravos de março até hoje. Organizado por ID do paciente, index de 0 a n-1
...
for i in tabelaPositivos.index: #Passando por cada positivo
    ...
    for j in notificacoesPaciente.index:
        ...
        if verificar.iloc[0]["Data da Notificação"] - timedelta(days=paramDiasReinfeccao) < referencia.iloc[0]["Data da Notificação"]: #A notificação verificada tem menos de 5 meses
