...
tabelaTotal = pd.read_excel("lista total.xlsx").sort_values(by="Cód. Paciente", ignore_index=True) #Lista total dos agravos de março até hoje. Organizado por ID do paciente, index de 0 a n-1
...
for i in tabelaSuspeitos.index: #Passando por cada suspeito
    ...
    elif "NEGATIVO" in situacoes["Situação"].values: #Se houver um negativo nas situações
        for j in situacoes.index: #Passa por cada situação
            if situacoes["Situação"][j] != "NEGATIVO": #Se a situação não for Negativo, ignora
                continue
            if situacoes["Data da Notificação"][j] >= (tabelaSuspeitos["Data da Notificação"][i] - timedelta(days=10)):
